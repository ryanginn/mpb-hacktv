...
int av_ffmpeg_open(vid_t *s, char *input_url, char *format, char *options)
{
	...
	if(av->audio_stream != NULL)
	{
		...
		if(!av->audio_codec_ctx->ch_layout.nb_channels)
		{
			...
			av_ch_layout_default(&default_ch_layout, av->audio_codec_ctx->ch_layout.nb_channels);
			av->audio_codec_ctx->ch_layout = default_ch_layout;
		}
		
		av_opt_set_int(av->swr_ctx, "in_ch_layout",    s->conf.downmix ? AV_CH_LAYOUT_STEREO : av->audio_codec_ctx->ch_layout.u.mask, 0);
		...
		av_opt_set_int(av->swr_ctx, "out_ch_layout",    AV_CH_LAYOUT_STEREO, 0);
